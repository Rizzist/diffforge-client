...
import { modelBrandFor } from "./modelBrandCatalog.js";

/* Brand marks for the model a session is currently on. The resolution table
   (substring detection over model id, provider id as fallback) lives in the
   JSX-free modelBrandCatalog.js so it can be unit-tested; this module owns
   the glyphs and the rendered marker. Unknown families fall back to the
   plain status dot.

   Real vendor SVGs ship only where a faithful mark is available:
   OpenAI comes from @styled-icons/simple-icons; DeepSeek is that project's
   official whale path. Claude/Gemini/Grok use the vendors' own published
   glyph geometry (Anthropic sunburst, Gemini four-point spark, xAI slash).
   Everyone else keeps the neutral colored-letter tile — we never invent a
   fake logo for a brand we don't have a real mark for. */

export { modelBrandFor };
