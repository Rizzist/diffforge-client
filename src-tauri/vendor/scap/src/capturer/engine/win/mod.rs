...
impl GraphicsCaptureApiHandler for Capturer {
    ...
    fn on_frame_arrived(
        ...
        // windows-capture 1.5.0 (crates.io) has no Frame::timespan; sample the
        // performance counter at arrival instead. Marginally later than the
        // frame's own capture timestamp, which is fine for display_time.
        let elapsed = unsafe {
            let mut time = 0;
            QueryPerformanceCounter(&mut time);
            time
