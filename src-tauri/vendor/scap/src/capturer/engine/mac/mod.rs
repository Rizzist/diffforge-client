...
pub fn get_output_frame_size(options: &Options) -> [u32; 2] {
    ...
    let mut output_width = (source_rect.size.width * scale_factor).round().max(1.0) as u32;
    let mut output_height = (source_rect.size.height * scale_factor).round().max(1.0) as u32;
    // 1200x800
