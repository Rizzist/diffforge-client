...
const PSEUDOCONSOLE_DEFAULT_FLAGS: DWORD = PSUEDOCONSOLE_INHERIT_CURSOR
    | PSEUDOCONSOLE_RESIZE_QUIRK
    | PSEUDOCONSOLE_WIN32_INPUT_MODE;
const PSEUDOCONSOLE_PASSTHROUGH_FLAGS: DWORD =
    PSEUDOCONSOLE_DEFAULT_FLAGS | PSEUDOCONSOLE_PASSTHROUGH_MODE;
static PSEUDOCONSOLE_PASSTHROUGH_FALLBACKS: AtomicUsize = AtomicUsize::new(0);
...
impl PsuedoCon {
    pub fn new(size: COORD, input: FileDescriptor, output: FileDescriptor) -> Result<Self, Error> {
        ...
        let passthrough_result = unsafe {
            (CONPTY.CreatePseudoConsole)(
                size,
                input.as_raw_handle() as _,
                output.as_raw_handle() as _,
                PSEUDOCONSOLE_PASSTHROUGH_FLAGS,
                &mut con,
            )
        };
        let result = if passthrough_result == S_OK {
            passthrough_result
        } else {
            PSEUDOCONSOLE_PASSTHROUGH_FALLBACKS.fetch_add(1, Ordering::Relaxed);
            log::warn!(
                "failed to create conpty with passthrough mode: HRESULT {}; retrying without passthrough",
                passthrough_result
            );
            unsafe {
                (CONPTY.CreatePseudoConsole)(
            ...
            result == S_OK,
            "failed to create psuedo console: HRESULT {} (passthrough HRESULT {})",
            result,
            passthrough_result
        );
        Ok(Self { con })
