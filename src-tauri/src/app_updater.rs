...
// hours, downloads in the background only when the user asks, and NEVER
// restarts on its own — terminals host live agent sessions, so the swap
// happens only from the explicit "restart to update" affordance.
//
...
fn app_update_settings_save(app: &AppHandle, auto_restart_when_idle: bool) -> Result<Value, String> {
...
fn app_update_status_snapshot() -> Value {
    ...
    json!({
        ...
        "version": available.as_ref().map(|info| info.version.clone()),
    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            let pending = match app_updater_run_check(&app).await {
                ...
                Ok(None) => false,
                Err(error) => {
                    log_terminal_status_event("backend.app_update.check_failed", json!({ "error": error }));
                    false
                }
            };
...
async fn app_update_install_and_restart(app: AppHandle) -> Result<(), String> {
    ...
    if let Err(error) = &result {
        ...
        log_terminal_status_event("backend.app_update.install_failed", json!({ "error": error }));
    }
    result
}
...
async fn app_update_install_inner(app: &AppHandle) -> Result<(), String> {
    ...
    // download_and_install; on macOS/Linux the swapped bundle only takes
    // effect after this relaunch.
