...
const TOKENOMICS_USAGE_EVENT_PRUNE_VACUUM_DONE_META_KEY: &str =
...
fn tokenomics_periodic_gate_claim_realtime_scan_slot() -> Result<(), u64> {
    ...
    if state.last_realtime_scan_ms > 0
        && now.saturating_sub(state.last_realtime_scan_ms) < TOKENOMICS_PERIODIC_REALTIME_SCAN_MIN_INTERVAL_MS
    {
...
fn tokenomics_maybe_prune_usage_events_for_app(app: &AppHandle) -> Result<Value, String> {
    ...
    let db_path = tokenomics_db_path(app)?;
    let mut conn = tokenomics_open_db(app)?;
    let vacuum_pending = tokenomics_meta_string(
        &conn,
...
fn tokenomics_prune_usage_events(
    ...
    let retention_cutoff_unix =
        now_unix.saturating_sub(TOKENOMICS_USAGE_EVENT_RETENTION_DAYS * 86_400);
    let (_, retention_cutoff_hour) =
        tokenomics_utc_hour_bucket_from_unix(retention_cutoff_unix);
...
fn tokenomics_capture_prune_source_import_identities(
    ...
    for capture in captures {
        transaction
            ...
            .map_err(|error| {
                format!("Unable to capture pruned Tokenomics source identity: {error}")
            })?;
    }
    Ok(())
}
...
fn tokenomics_prune_usage_event_chunk(
    ...
    let deleted = tokenomics_with_db_write_transaction(
        ...
        |transaction| {
            tokenomics_capture_prune_source_import_identities(transaction, now.as_str())?;
                ...
                .execute(
                    "INSERT INTO tokenomics_pruned_usage_rollups(
                     ...
                     SELECT
                       ...
                       NULLIF(e.subscription_key, '') AS subscription_key,
                       NULLIF(e.provider_account_key, '') AS provider_account_key,
                       MAX(NULLIF(e.provider_account_label, '')) AS provider_account_label,
                       COALESCE(NULLIF(e.billing_scope_type, ''), 'unknown') AS billing_scope_type,
                       NULLIF(e.billing_team_id, '') AS billing_team_id,
                       MAX(COALESCE(NULLIF(e.billing_scope_source, ''), 'unknown')) AS billing_scope_source,
                       NULLIF(e.workspace_id, '') AS workspace_id,
                       MAX(NULLIF(e.repo_path, '')) AS repo_path,
                       'hour' AS bucket_width,
                       e.bucket_hour AS bucket_start,
                       COALESCE(SUM(e.input_tokens), 0) AS input_tokens,
                       COALESCE(SUM(e.output_tokens), 0) AS output_tokens,
                       COALESCE(SUM(e.cache_read_tokens), 0) AS cache_read_tokens,
                       COALESCE(SUM(e.cache_write_tokens), 0) AS cache_write_tokens,
                     ...
                     JOIN tokenomics_rollups r
                      ...
                      AND r.agent_kind=e.agent_kind
                      AND COALESCE(r.model, '')=COALESCE(NULLIF(e.model, ''), '')
                      AND COALESCE(r.subscription_key, '')=COALESCE(NULLIF(e.subscription_key, ''), '')
                      AND COALESCE(r.provider_account_key, '')=COALESCE(NULLIF(e.provider_account_key, ''), '')
                      AND COALESCE(NULLIF(r.billing_scope_type, ''), 'unknown')=COALESCE(NULLIF(e.billing_scope_type, ''), 'unknown')
                      AND COALESCE(r.billing_team_id, '')=COALESCE(NULLIF(e.billing_team_id, ''), '')
                      AND COALESCE(r.workspace_id, '')=COALESCE(NULLIF(e.workspace_id, ''), '')
                      AND r.bucket_start=e.bucket_hour
                     GROUP BY
                       r.id, e.device_id, e.provider, e.agent_kind,
                       NULLIF(e.model, ''), NULLIF(e.subscription_key, ''),
                       NULLIF(e.provider_account_key, ''),
                       COALESCE(NULLIF(e.billing_scope_type, ''), 'unknown'),
                       NULLIF(e.billing_team_id, ''), NULLIF(e.workspace_id, ''),
                       e.bucket_hour
...
fn tokenomics_maybe_vacuum_after_usage_event_prune(
    ...
    let final_size = fs::metadata(db_path).map(|metadata| metadata.len()).unwrap_or(0);
    log_terminal_status_event(
...
fn tokenomics_active_provider_account_key_map(limits: &[Value]) -> HashMap<String, String> {
    ...
    for limit in limits {
        ...
        let agent_kind = tokenomics_value_string(limit, &["agent_kind"])
...
fn tokenomics_opencode_account_key_identifiers(auth_value: &Value) -> Vec<String> {
    ...
    key.map(|key| vec![tokenomics_hash(key)])
        .unwrap_or_default()
}
...
fn tokenomics_merge_source_identity_usage_candidate(
    candidates: &mut HashMap<(String, String, Option<String>), TokenomicsSourceIdentityUsageCandidate>,
...
fn tokenomics_delete_provider_rows(
    ...
    conn.execute(
        "DELETE FROM tokenomics_pruned_usage_rollups WHERE provider=?1 AND agent_kind=?2",
        rusqlite::params![provider, agent_kind],
    )
    .map_err(|error| format!("Unable to clear pruned Tokenomics provider rollups: {error}"))?;
    conn.execute(
        "DELETE FROM tokenomics_usage_event_tombstones WHERE provider=?1 AND agent_kind=?2",
        rusqlite::params![provider, agent_kind],
    )
    .map_err(|error| format!("Unable to clear Tokenomics provider tombstones: {error}"))?;
    conn.execute(
        "DELETE FROM tokenomics_source_imports WHERE provider=?1 AND agent_kind=?2",
        rusqlite::params![provider, agent_kind],
    )
    .map_err(|error| format!("Unable to clear Tokenomics provider source imports: {error}"))?;
    Ok(())
}
...
fn tokenomics_rekey_pruned_usage_rollups(
    ...
    let mut statement = conn
        ...
        .map_err(|error| format!("Unable to prepare pruned Tokenomics rollup rekey: {error}"))?;
    let rows = statement
        ...
        .map_err(|error| format!("Unable to query pruned Tokenomics rollup rekey: {error}"))?;
    let mut rekey_rows = Vec::new();
    for row in rows {
        rekey_rows.push(
            row.map_err(|error| format!("Unable to read pruned Tokenomics rollup rekey: {error}"))?,
        );
    }
    drop(statement);
    ...
    for row in rekey_rows {
        ...
        let updated = conn
            .execute(
                "UPDATE tokenomics_pruned_usage_rollups
                 ...
                 WHERE id=?2
                   AND NOT EXISTS (
                     SELECT 1 FROM tokenomics_pruned_usage_rollups existing
                     WHERE existing.id=?1
                   )",
                rusqlite::params![new_id.as_str(), row.old_id.as_str()],
            )
            .map_err(|error| format!("Unable to rekey pruned Tokenomics rollup id: {error}"))?;
        if updated > 0 {
        ...
        .map_err(|error| format!("Unable to merge pruned Tokenomics rollup rekey: {error}"))?;
        conn.execute(
            "DELETE FROM tokenomics_pruned_usage_rollups WHERE id=?1",
            rusqlite::params![row.old_id.as_str()],
        )
        .map_err(|error| format!("Unable to remove stale pruned Tokenomics rollup id: {error}"))?;
...
fn tokenomics_migrate_provider_account_key(
    ...
    if events_by_subscription {
        changed_events = changed_events.saturating_add(
            ...
            .map_err(|error| format!("Unable to migrate Tokenomics subscription events: {error}"))?,
        );
    }
    ...
    if pruned_by_account {
        changed_pruned = changed_pruned.saturating_add(
            ...
            .map_err(|error| format!("Unable to migrate pruned Tokenomics account rollups: {error}"))?,
        );
    }
    ...
    if changed_events > 0 || changed_pruned > 0 || rollups_by_account || rollups_by_subscription {
...
fn tokenomics_rewrite_cloud_provider_limits_for_account_key(
    ...
    for limit in &mut limits {
        let row_provider =
            tokenomics_value_string(limit, &["provider"]).unwrap_or_else(|| "unknown".to_string());
        let row_agent = tokenomics_value_string(limit, &["agent_kind"])
...
fn tokenomics_reconcile_fingerprint_stat(
    ...
    conn.query_row(&sql, [], |row| {
        ...
        let max_value: String = row
            .get::<_, rusqlite::types::Value>(1)
            .map(|value| match value {
                ...
                rusqlite::types::Value::Blob(value) => tokenomics_hash(&String::from_utf8_lossy(&value)),
            })?;
...
fn tokenomics_current_provider_accounts_reconcile_fingerprint(
    ...
    let codex_alias = tokenomics_read_codex_usage_cache_alias(conn, &codex_account.key)
        .unwrap_or_default();
    let mut claude_legacy_keys = tokenomics_claude_legacy_account_keys();
    claude_legacy_keys.sort();
    claude_legacy_keys.dedup();
    let parts = vec![
        format!("codex:{}:{}:{}", codex_account.key, codex_account.label, codex_alias),
        format!(
            "claude:{}:{}:{}",
            claude_account.key,
...
fn tokenomics_delete_source_usage_events_without_rollup_rebuild(
    ...
    let deleted = conn
        ...
        .map_err(|error| format!("Unable to clear Tokenomics source events: {error}"))?;
    Ok(deleted)
}
...
fn tokenomics_collect_source_rollup_keys_for_match(
    ...
    let mut keys = Vec::new();
    for row in rows {
        keys.push(row.map_err(|error| format!("Unable to read Tokenomics source rollup key: {error}"))?);
    }
    Ok(keys)
}
...
fn tokenomics_delete_source_usage_events_collecting_rollup_keys(
    ...
    let deleted =
        tokenomics_delete_source_usage_events_without_rollup_rebuild(conn, provider, agent_kind, path)?;
    Ok((deleted, keys))
}
...
fn tokenomics_delete_source_usage_events(
    ...
    if deleted > 0 {
        keys.extend(tokenomics_source_rollup_keys(conn, provider, agent_kind, path)?);
...
fn tokenomics_upsert_source_import(
    ...
    let source_identity = tokenomics_source_identity_from_usage_events(conn, provider, agent_kind, path)?;
...
fn tokenomics_rebuild_provider_rollups_for_scoped_hours(
    ...
    let mut keys = keys.into_iter().collect::<HashSet<_>>().into_iter().collect::<Vec<_>>();
    if keys.is_empty() {
        return Ok(0);
    }
    ...
    let _heavy_permit = if tokenomics_db_write_lock_held_by_thread() {
        ...
        Some(backend_heavy_job_acquire("tokenomics.rollups.scoped_rebuild"))
    };
    ...
    for key in keys {
        ...
        let rows = statement
            ...
            .map_err(|error| format!("Unable to query scoped Tokenomics rollup rebuild: {error}"))?;
...
fn tokenomics_repair_provider_api_costs(conn: &rusqlite::Connection) -> Result<(), String> {
    ...
    for row in rows {
        let (
            ...
            cache_read_tokens,
            cache_write_tokens,
        ) = row.map_err(|error| format!("Unable to read pruned Tokenomics cost repair row: {error}"))?;
        let estimated_cost_microusd = tokenomics_estimated_api_microusd(
            &provider,
            &agent_kind,
            model.as_deref(),
...
fn tokenomics_rebuild_provider_rollups_for_width(
    ...
    let rows = statement
        .query_map(rusqlite::params![provider, agent_kind, bucket_width], |row| {
            Ok((
                row.get::<_, String>(0)?,
                row.get::<_, String>(1)?,
                row.get::<_, String>(2)?,
...
fn tokenomics_upsert_latest_window(
    ...
    let agent_kind = tokenomics_value_string(value, &["agent_kind"])
        .unwrap_or_else(|| provider.clone());
    ...
    let provider_account_label =
        tokenomics_value_string(value, &["provider_account_label"])
            .unwrap_or_else(|| fallback_account.label.clone());
    ...
    let pace_status =
        tokenomics_value_string(value, &["pace_status"]).unwrap_or_default();
    let pace_delta_percent =
        tokenomics_value_i64(value, &["pace_delta_percent"]);
    ...
    let device_id =
        tokenomics_value_string(value, &["device_id", "machine_id"])
    ...
    let material_changed = match conn.query_row(
        ...
        |row| {
            Ok(row.get::<_, Option<String>>(0)? != Some(provider_account_label.clone())
...
fn tokenomics_recent_provider_limit_samples_for_limit(
    ...
    .unwrap_or_else(|| "5_hour".to_string());
    let device_id =
        tokenomics_value_string(limit, &["device_id", "machine_id"])
            .unwrap_or_default();
    let now_unix = tokenomics_unix_now();
...
fn tokenomics_apply_provider_limit_sample_pacing_from_rows(limit: &mut Value, samples: &[Value]) {
    ...
    let window_seconds = tokenomics_limit_effective_window_seconds(
        &window_kind,
        tokenomics_value_i64(latest, &["limit_window_seconds"]).or_else(
            || tokenomics_value_i64(limit, &["limit_window_seconds"]),
        ),
    )
    .max(1) as u64;
...
fn tokenomics_historical_prune_ack_candidate_days(
    ...
    let after_hour = after_day_start_ms
    ...
    let mut statement = conn
        ...
        .map_err(|error| format!("Unable to prepare historical Tokenomics prune ack days: {error}"))?;
    let rows = statement
        .query_map(
            rusqlite::params![
                TOKENOMICS_USAGE_EVENT_PRUNE_ACK_DAY_META_PREFIX,
        ...
        .map_err(|error| format!("Unable to query historical Tokenomics prune ack days: {error}"))?;
...
fn tokenomics_normalize_cloud_relay_summary(summary: &Value) -> Value {
    ...
    let inherited_device_id = tokenomics_text_field(
        summary,
        &["device_id", "machine_id"],
    );
    ...
    for group in tokenomics_v2_collection_values(
        ...
        let Some(bucket_ms) = tokenomics_value_i64(group, &["bucket_start_ms"])
        else {
            continue;
        };
...
fn tokenomics_cloud_relay_hourly_group_row(
    ...
    let device_id =
        tokenomics_text_field(row, &["device_id", "machine_id"])
            .or_else(|| inherited_device_id.map(str::to_string));
    let provider =
        tokenomics_value_string(row, &["provider"]).unwrap_or_else(|| "unknown".to_string());
    let agent_kind = tokenomics_value_string(row, &["agent_kind"])
        .unwrap_or_else(|| provider.clone());
    let account_key = tokenomics_value_string(
        row,
        &[
...
fn tokenomics_cloud_relay_window_row(
    ...
    let device_id = tokenomics_text_field(
        window,
        &["device_id", "machine_id"],
    )
    .or_else(|| inherited_device_id.map(str::to_string));
    ...
    let agent_kind = tokenomics_value_string(window, &["agent_kind"])
        .unwrap_or_else(|| provider.clone());
    let account_key = tokenomics_value_string(
        window,
        &[
...
fn tokenomics_account_device_live_state_payload(event: &Value) -> Option<&Value> {
    let event_kind =
        tokenomics_text_field(event, &["event_kind", "kind"]).unwrap_or_default();
...
fn tokenomics_extend_v2_provider_account_usage(
    ...
    let agent_kind =
        tokenomics_v2_account_text(account, root_account, &["agent_kind"])
...
fn tokenomics_v2_hourly_row(
    ...
    let bucket_ms =
        tokenomics_value_i64(row, &["bucket_start_ms"]).or_else(|| {
...
fn tokenomics_v2_window_row(
    ...
    let used_percent = tokenomics_value_i64(window, &["used_percent", "used"])
...
mod tokenomics_tests {
    ...
    fn tokenomics_usage_event_prune_policy_requires_age_sync_and_rollup_coverage() {
        ...
        assert!(!tokenomics_usage_event_prune_policy_allows(young, now, true, true));
        assert!(tokenomics_usage_event_prune_policy_allows(old, now, true, true));
        ...
        assert!(!tokenomics_usage_event_prune_policy_allows(old, now, false, true));
        assert!(!tokenomics_usage_event_prune_policy_allows(
    ...
    fn tokenomics_scoped_rollup_rebuild_matches_full_with_pruned_contributions() {
        ...
        tokenomics_prepare_db(&conn).unwrap();
        let (bucket_day, bucket_hour) = tokenomics_utc_hour_bucket_from_unix(1_700_000_000);
        ...
        let scoped_total: (i64, i64, i64) = conn
            ...
            .unwrap();
        assert_eq!(scoped_total, (27, 27, 3));
    ...
    #[test]
    fn tokenomics_codex_import_ledger_repair_prunes_orphaned_event_rows() {
        let conn = rusqlite::Connection::open_in_memory().unwrap();
        tokenomics_prepare_db(&conn).unwrap();
        let bucket_start = tokenomics_test_current_hour_bucket();
        let day_start = bucket_start.get(0..10).unwrap_or("1970-01-01").to_string();
        ...
        .unwrap();
        assert_eq!(rollup["input_tokens"], json!(5));
        assert_eq!(rollup["total_tokens"], json!(6));
    }
    ...
    fn tokenomics_source_offset_resume_rejects_rewritten_prefix() {
        ...
        let second_scan =
            tokenomics_scan_file(&conn, "anthropic", "claude", &account, &path, -1, 0).unwrap();
        assert_eq!(second_scan.inserted_events, 2);
        let totals = tokenomics_query_one(
            &conn,
            "SELECT COUNT(*) AS count, COALESCE(SUM(total_tokens), 0) AS total_tokens
             FROM tokenomics_usage_events
        ...
        .unwrap();
        assert_eq!(totals["count"], json!(2));
        assert_eq!(totals["total_tokens"], json!(28));
    }
