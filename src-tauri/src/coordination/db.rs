...
fn apply_workspace_mcp_secrets_state_migration(
    ...
    if migration_applied(connection, WORKSPACE_MCP_SECRETS_STATE_MIGRATION_VERSION)? {
        return Ok(SchemaMigrationDiagnostics::new(
            WORKSPACE_MCP_SECRETS_STATE_MIGRATION_VERSION,
            WORKSPACE_MCP_SECRETS_STATE_MIGRATION_NAME,
            "already_applied",
    ...
    with_sqlite_lock_retry(
        "Unable to initialize workspace MCP secrets state schema",
        || connection.execute_batch(WORKSPACE_MCP_SECRETS_STATE_SCHEMA_SQL),
    )?;
    ...
    migration.details.splice(
        0..0,
        ["WORKSPACE_MCP_SECRETS_STATE_SCHEMA_SQL executed idempotently".to_string()],
    );
    Ok(migration)
}

fn apply_workspace_mcp_seed_state_migration(
    connection: &Connection,
) -> Result<SchemaMigrationDiagnostics, String> {
    if migration_applied(connection, WORKSPACE_MCP_SEED_STATE_MIGRATION_VERSION)? {
        return Ok(SchemaMigrationDiagnostics::new(
    ...
    migration.details.splice(
        0..0,
        ["WORKSPACE_MCP_SEED_STATE_SCHEMA_SQL executed idempotently".to_string()],
    );
    Ok(migration)
}
