...
fn tool_description(name: &str) -> String {
    match name {
        ...
        "architecture_context" => "Return the repo-scoped Diff Forge architecture contract, storage paths, DSL rules, existing graph summaries, compact actor-node guidance, and icon-reference path. Call this before architecture, diagram, deployment, flow, or subsystem visualization work, then edit .agents/architectures/graphs/*.arch directly so the Architecture tab reloads file changes live.".to_string(),
