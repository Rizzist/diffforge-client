...
struct CloudMcpState {
    ...
    spec_graph_container_syncs: Arc<Mutex<HashMap<String, CloudMcpSpecGraphContainerRuntime>>>,
...
#[derive(Clone)]
struct CloudMcpSpecGraphContainerRuntime {
    generation: u64,
...
impl CloudMcpState {
    fn new() -> Self {
        ...
        Self {
            ...
            spec_graph_container_syncs: Arc::new(Mutex::new(HashMap::new())),
...
fn cloud_mcp_prepare_filetree_sync_seeds(
    ...
    let primary = cloud_mcp_prepare_filetree_sync_seed_from_root(
        root,
        workspace_id.clone(),
        workspace_name,
    );
...
fn cloud_mcp_filetree_snapshot_payload(
    ...
    payload.insert("workspace_root".to_string(), json!(workspace_root_display.clone()));
...
async fn cloud_mcp_push_filetree_snapshot(
    ...
    let sync_session = sync_session.cloned().unwrap_or_else(|| {
        cloud_mcp_filetree_sync_session(repo_id, workspace_root, reason, 0)
    });
...
async fn cloud_mcp_push_filetree_sync_begins_for_workspace_request(
    ...
    for seed in seeds {
        let mut session =
            cloud_mcp_filetree_sync_session(&seed.repo_id, &seed.root, reason, 1);
...
async fn cloud_mcp_push_current_filetree_snapshot(
    ...
    let project_mounts =
        cloud_mcp_project_mounts_for_workspace_sync(workspace_root, &workspace_kind, &detected_mounts);
...
async fn cloud_mcp_signal_desktop_closing(
    app: &AppHandle,
    reason: &str,
) -> Result<Value, String> {
...
async fn cloud_mcp_sync_terminal_status_event(
    ...
    let event_type = cloud_mcp_payload_text(
        &terminal,
        &["event_type", "eventType", "type", "terminal_event_type", "terminalEventType"],
    )
...
fn cloud_mcp_attach_mount_metadata(
    ...
    object.insert("mount_kind".to_string(), json!(mount.mount_kind.clone()));
    object.insert("mountKind".to_string(), json!(mount.mount_kind.clone()));
    object.insert("parent_mount_id".to_string(), json!(mount.parent_mount_id.clone()));
    object.insert("parentMountId".to_string(), json!(mount.parent_mount_id.clone()));
    ...
    if let Some(metadata) = metadata.as_object_mut() {
        ...
        metadata.insert("mount_kind".to_string(), json!(mount.mount_kind.clone()));
        metadata.insert("mountKind".to_string(), json!(mount.mount_kind.clone()));
        metadata.insert("parent_mount_id".to_string(), json!(mount.parent_mount_id.clone()));
        metadata.insert("parentMountId".to_string(), json!(mount.parent_mount_id.clone()));
...
fn cloud_mcp_container_mount_node(
    ...
    let is_container_mount = mount.mount_kind == "container";
    let project_label = if is_container_mount {
        "container"
    } else if mount.has_git {
        "Git project"
    } else {
        "project"
    };
    let summary = if mount.has_agents {
        format!("Mounted {project_label} with local Diff Forge state.")
    } else {
        format!("Mounted {project_label}.")
    };
    json!({
        "id": id,
        ...
        "specs": [],
        "mount_id": mount.mount_id.clone(),
        "mount_kind": mount.mount_kind.clone(),
        "parent_mount_id": mount.parent_mount_id.clone(),
        "mount_depth": mount.mount_depth,
        ...
        "metadata": {
            "mount_id": mount.mount_id.clone(),
            "mountKind": mount.mount_kind.clone(),
            "mount_kind": mount.mount_kind.clone(),
            "parentMountId": mount.parent_mount_id.clone(),
            "parent_mount_id": mount.parent_mount_id.clone(),
            "mountDepth": mount.mount_depth,
            "mount_depth": mount.mount_depth,
            "workspace_root": workspace_path_display(workspace_root),
            "project_root": mount.project_root.clone(),
...
fn cloud_mcp_prefix_container_node(
    ...
    mount: &WorkspaceProjectMount,
    source_repo_id: &str,
    source_graph_cursor: Option<&str>,
    source_node_hash: Option<&str>,
    mut node: Value,
    index: usize,
    ...
    let source_node_id = object
        .get("sourceNodeId")
        .or_else(|| object.get("source_node_id"))
        .and_then(Value::as_str)
        .map(str::to_string)
        .unwrap_or_else(|| original_id.clone());
    let immediate_source_repo_id = source_repo_id.to_string();
    let source_repo_id = object
        .get("sourceRepoId")
        .or_else(|| object.get("source_repo_id"))
        .and_then(Value::as_str)
        .map(str::to_string)
        .unwrap_or_else(|| immediate_source_repo_id.clone());
    object.insert("id".to_string(), json!(prefixed_id.clone()));
    object.insert("container_node_id".to_string(), json!(prefixed_id.clone()));
    object.insert("containerNodeId".to_string(), json!(prefixed_id.clone()));
    object.insert("source_node_id".to_string(), json!(source_node_id.clone()));
    object.insert("sourceNodeId".to_string(), json!(source_node_id.clone()));
    object.insert("source_repo_id".to_string(), json!(source_repo_id.clone()));
    object.insert("sourceRepoId".to_string(), json!(source_repo_id.clone()));
    object.insert("immediate_source_node_id".to_string(), json!(original_id.clone()));
    object.insert("immediateSourceNodeId".to_string(), json!(original_id.clone()));
    object.insert(
        "immediate_source_repo_id".to_string(),
        json!(immediate_source_repo_id.clone()),
    );
    object.insert(
        "immediateSourceRepoId".to_string(),
        json!(immediate_source_repo_id.clone()),
    );
    if let Some(cursor) = source_graph_cursor.filter(|cursor| !cursor.trim().is_empty()) {
        object.insert("source_graph_cursor".to_string(), json!(cursor));
        object.insert("sourceGraphCursor".to_string(), json!(cursor));
    }
    if let Some(hash) = source_node_hash.filter(|hash| !hash.trim().is_empty()) {
        object.insert("source_node_hash".to_string(), json!(hash));
        object.insert("sourceNodeHash".to_string(), json!(hash));
    }
    ...
    if let Some(metadata) = object.get_mut("metadata").and_then(Value::as_object_mut) {
        metadata.insert("container_node_id".to_string(), json!(prefixed_id.clone()));
        metadata.insert("containerNodeId".to_string(), json!(prefixed_id));
        metadata.insert("source_node_id".to_string(), json!(source_node_id.clone()));
        metadata.insert("sourceNodeId".to_string(), json!(source_node_id));
        metadata.insert("source_repo_id".to_string(), json!(source_repo_id.clone()));
        metadata.insert("sourceRepoId".to_string(), json!(source_repo_id));
        metadata.insert(
            "immediate_source_repo_id".to_string(),
            json!(immediate_source_repo_id.clone()),
        );
        metadata.insert(
            "immediateSourceRepoId".to_string(),
            json!(immediate_source_repo_id),
        );
        if let Some(cursor) = source_graph_cursor.filter(|cursor| !cursor.trim().is_empty()) {
            metadata.insert("source_graph_cursor".to_string(), json!(cursor));
            metadata.insert("sourceGraphCursor".to_string(), json!(cursor));
        }
        if let Some(hash) = source_node_hash.filter(|hash| !hash.trim().is_empty()) {
            metadata.insert("source_node_hash".to_string(), json!(hash));
            metadata.insert("sourceNodeHash".to_string(), json!(hash));
        }
    }
...
fn cloud_mcp_selected_root_has_project_graph(root: &Path) -> bool {
    let mounts = workspace_project_mounts(root);
    workspace_selected_root_mount(root, &mounts).is_some()
}

fn cloud_mcp_relative_spec_node_path(root: &Path, node: &Value) -> String {
    let raw_path = cloud_mcp_payload_text(node, &["path", "display_path", "displayPath"])
        .unwrap_or_default();
    ...
    if path.is_absolute() {
        if let Some(relative) = child_relative_path(root, &path) {
            return relative;
        }
    }

    normalize_git_status_path(trimmed)
}

fn cloud_mcp_path_is_inside_mount(relative_path: &str, mount_paths: &[String]) -> bool {
    let relative_path = normalize_git_status_path(relative_path);
    !relative_path.is_empty()
        && mount_paths.iter().any(|mount_path| {
            relative_path == *mount_path
                || relative_path.starts_with(&format!("{mount_path}/"))
        })
}

fn cloud_mcp_append_parent_spec_graph(
    req: &CloudMcpSpecGraphSyncRequest,
    mounts: &[WorkspaceProjectMount],
    container_node_id: &str,
    nodes: &mut Vec<Value>,
    edges: &mut Vec<Value>,
    task_history_tasks: &mut Vec<Value>,
    cursor_parts: &mut Vec<String>,
) {
    if !cloud_mcp_selected_root_has_project_graph(&req.root) {
        return;
    }

    let Some(parent_snapshot) = cloud_mcp_existing_spec_graph_cache(req) else {
        return;
    };
    let mount_paths = mounts
        ...
        .filter(|path| !path.is_empty())
        .collect::<Vec<_>>();
    let parent_cursor = parent_snapshot
        .get("cursor")
        .and_then(Value::as_str)
        .unwrap_or_default();
    if !parent_cursor.is_empty() {
        cursor_parts.push(parent_cursor.to_string());
    }

    let parent_nodes = cloud_mcp_spec_graph_array(&parent_snapshot, "specNodes", "nodes");
    let parent_edges = cloud_mcp_spec_graph_array(&parent_snapshot, "specEdges", "edges");
    let mut kept_node_ids = HashSet::new();
    let mut first_parent_node_id: Option<String> = None;
    let mut root_like_parent_node_id: Option<String> = None;

    for mut node in parent_nodes {
        let Some(node_id) = cloud_mcp_spec_graph_item_id(&node) else {
            continue;
        };
        let relative_path = cloud_mcp_relative_spec_node_path(&req.root, &node);
        if cloud_mcp_path_is_inside_mount(&relative_path, &mount_paths) {
            continue;
        }
        ...
        if let Some(object) = cloud_mcp_value_object_mut(&mut node) {
            object.insert("source_repo_id".to_string(), json!(req.repo_id.clone()));
            object.insert("sourceRepoId".to_string(), json!(req.repo_id.clone()));
            object.insert("project_root".to_string(), json!(req.root_display.clone()));
            object.insert("projectRoot".to_string(), json!(req.root_display.clone()));
            object.insert("local_parent_repo".to_string(), json!(true));
            object.insert("localParentRepo".to_string(), json!(true));
        }
        if first_parent_node_id.is_none() {
            first_parent_node_id = Some(node_id.clone());
        }
        if root_like_parent_node_id.is_none() && is_root_like {
            root_like_parent_node_id = Some(node_id.clone());
        }
        kept_node_ids.insert(node_id);
        nodes.push(node);
    }

    if let Some(parent_root_id) = root_like_parent_node_id.or(first_parent_node_id) {
        edges.push(json!({
            "id": format!("container-parent-root-edge-{parent_root_id}"),
            "from": container_node_id,
            "to": parent_root_id,
            "kind": "contains",
            "metadata": {
                "containment": true,
                "source": "parent_repo_graph",
                "repo_id": req.repo_id.clone(),
                "project_root": req.root_display.clone(),
            }
        }));
    }

    for edge in parent_edges {
        let Some(from) = cloud_mcp_payload_text(&edge, &["from_node_id", "fromNodeId", "from", "source"]) else {
            continue;
        };
        let Some(to) = cloud_mcp_payload_text(&edge, &["to_node_id", "toNodeId", "to", "target"]) else {
            continue;
        };
        if kept_node_ids.contains(&from) && kept_node_ids.contains(&to) {
            edges.push(edge);
        }
    }

    if let Some(tasks) = parent_snapshot
        .get("taskHistory")
        .or_else(|| parent_snapshot.get("raw").and_then(|raw| raw.get("task_history")))
        .and_then(|history| history.get("tasks"))
        .and_then(Value::as_array)
    {
        task_history_tasks.extend(tasks.iter().cloned());
    }
}
...
fn cloud_mcp_container_spec_graph_snapshot(
    ...
    let manifest_mounts = workspace_mount_manifest_from_projects(&req.root, mounts);
    let mount_values = manifest_mounts
        .iter()
        .map(|mount| {
            json!({
                "mountId": mount.mount_id.clone(),
                ...
                "projectKind": mount.project_kind.clone(),
                "hasGit": mount.has_git,
    ...
    let mut mount_node_ids = HashMap::new();

    for mount in &manifest_mounts {
        let mount_node_id = cloud_mcp_mount_node_id(mount);
        mount_node_ids.insert(mount.mount_id.clone(), mount_node_id.clone());
        nodes.push(cloud_mcp_container_mount_node(&req.root, &req.repo_id, mount));
    }

    for mount in &manifest_mounts {
        let mount_node_id = cloud_mcp_mount_node_id(mount);
        let parent_node_id = mount
            .parent_mount_id
            .as_ref()
            .and_then(|parent_mount_id| mount_node_ids.get(parent_mount_id))
            .cloned()
            .unwrap_or_else(|| container_node_id.clone());
        edges.push(json!({
    ...
    cloud_mcp_append_parent_spec_graph(
        req,
        mounts,
        &container_node_id,
        &mut nodes,
        &mut edges,
        &mut task_history_tasks,
        &mut cursor_parts,
    );
    ...
    for mount in mounts {
        ...
        let child_mounts = cloud_mcp_container_project_mounts(&project_req.root);
        let snapshot = if child_mounts.is_empty() {
            let Some(snapshot) = cloud_mcp_existing_spec_graph_cache(&project_req) else {
                continue;
            };
            snapshot
        } else {
            cloud_mcp_container_spec_graph_snapshot(
                &project_req,
                &child_mounts,
                sync_state,
                sync_error,
        ...
        let child_cursor = snapshot.get("cursor").and_then(Value::as_str);
        let child_node_hashes = snapshot
            .get("nodeHashes")
            .or_else(|| snapshot.get("node_hashes"))
            .and_then(Value::as_object);
        ...
        for (index, node) in child_nodes.into_iter().enumerate() {
            let original_id = cloud_mcp_spec_graph_item_id(&node)
                .unwrap_or_else(|| format!("node-{index}"));
            let source_node_hash = child_node_hashes
                .and_then(|hashes| hashes.get(&original_id))
                .and_then(Value::as_str);
            if let Some((node, prefixed_id, is_root_like)) =
                cloud_mcp_prefix_container_node(
                    &req.root,
                    mount,
                    &project_req.repo_id,
                    child_cursor,
...
async fn cloud_mcp_container_snapshots_for_child_sync(
    ...
    let child_root_key = normalized_path_key(child_root);
    let containers = {
        let syncs = state.spec_graph_container_syncs.lock().await;
        syncs
            .values()
            .filter(|runtime| {
                runtime.mounts.iter().any(|mount| {
                    normalized_path_key(&mount.root_path) == child_root_key
                })
            })
            .cloned()
            .collect::<Vec<_>>()
    ...
    containers
        .into_iter()
        .map(|runtime| {
            let mut snapshot = cloud_mcp_container_spec_graph_snapshot(
                &runtime.req,
                &runtime.mounts,
                sync_state,
                sync_error,
            ...
            if let Some(object) = snapshot.as_object_mut() {
                object.insert("syncGeneration".to_string(), json!(runtime.generation));
                object.insert(
                    "lastChildRepoPath".to_string(),
                    json!(workspace_path_display(child_root)),
                );
            }
            snapshot
        })
...
async fn cloud_mcp_container_snapshot_for_root_sync(
    state: &CloudMcpState,
    req: &CloudMcpSpecGraphSyncRequest,
    sync_state: &str,
    sync_error: &str,
    ...
    let runtime = {
        let syncs = state.spec_graph_container_syncs.lock().await;
        syncs.get(&req.repo_id).cloned()
    }?;
    Some(cloud_mcp_container_spec_graph_snapshot(
        &runtime.req,
        &runtime.mounts,
        sync_state,
        sync_error,
...
async fn cloud_mcp_emit_spec_graph_snapshot_and_parent_containers(
    app: &AppHandle,
    state: &CloudMcpState,
    req: &CloudMcpSpecGraphSyncRequest,
    snapshot: Value,
) {
    let sync_state = cloud_mcp_payload_text(&snapshot, &["syncState", "sync_state"])
        .unwrap_or_else(|| "local_mounts".to_string());
    let sync_error = cloud_mcp_payload_text(&snapshot, &["syncError", "sync_error"])
        .unwrap_or_default();
    cloud_mcp_emit_spec_graph_snapshot(app, snapshot);
    if let Some(mut composed_snapshot) =
        cloud_mcp_container_snapshot_for_root_sync(state, req, &sync_state, &sync_error).await
    {
        if let Some(object) = composed_snapshot.as_object_mut() {
            object.insert(
                "lastParentRepoPath".to_string(),
                json!(workspace_path_display(&req.root)),
            );
        }
        cloud_mcp_emit_spec_graph_snapshot(app, composed_snapshot);
    }
    for parent_snapshot in cloud_mcp_container_snapshots_for_child_sync(
        state,
        &req.root,
        &sync_state,
        &sync_error,
    ...
    .await
    {
        cloud_mcp_emit_spec_graph_snapshot(app, parent_snapshot);
    }
}
    ...
    match sync_result {
        Ok((snapshot, changed)) => {
            if *first_sync || changed || filetree_synced {
                cloud_mcp_emit_spec_graph_snapshot_and_parent_containers(
                    app,
                    state,
                    req,
                    snapshot,
        ...
        Err(error) => {
            ...
            cloud_mcp_emit_spec_graph_snapshot_and_parent_containers(app, state, req, snapshot)
                .await;
            retry
        }
    }
...
async fn cloud_mcp_stop_spec_graph_sync(
    ...
    let req = cloud_mcp_spec_graph_sync_request(repo_path.clone(), None, None);
    let container_removed = {
        ...
        if sync_generation
            ...
            .unwrap_or(true)
        {
            container_syncs.remove(&req.repo_id)
        } else {
            None
        }
    };
    let child_reqs = container_removed
        .as_ref()
        .map(|runtime| runtime.mounts.clone())
        .or_else(|| {
            if sync_generation.is_none() {
                Some(cloud_mcp_container_project_mounts(&req.root))
            } else {
                None
            }
        })
        ...
        .into_iter()
        .map(|mount| {
            cloud_mcp_spec_graph_sync_request(
                workspace_path_display(&mount.root_path),
                None,
                None,
            )
    ...
    Ok(json!({
        ...
        "repoId": req.repo_id.clone(),
        "repoPath": req.root_display.clone(),
        "stopped": stopped || container_removed.is_some(),
        "childSyncs": child_stopped,
    }))
}
...
\u{1b}[47;1H\u{1b}[22m\u{1b}[1m›\u{1b}[47;3H\u{1b}[22m\u{1b}[2mExplain this codebase";
    ...
    #[test]
    fn container_spec_graph_snapshot_prefixes_child_nodes_with_routing_metadata() {
        let root = test_cloud_root("diffforge-cloud-container-spec-graph");
        let frontend = root.join("frontend");
        create_cloud_package_project(&frontend);

        let child_req = cloud_mcp_spec_graph_sync_request(
            workspace_path_display(&frontend),
            Some("workspace-test".to_string()),
            Some("frontend".to_string()),
        );
        let child_cache_path = cloud_mcp_spec_graph_cache_path(&child_req.root, &child_req.repo_id);
        let child_snapshot = cloud_mcp_spec_graph_snapshot_from_data(
            &child_req,
            json!({
                "nodes": [{
                    "id": "file-app",
                    ...
                    "title": "App",
                    "path": "src/App.jsx",
                    "active_specs": [{"id": "spec-1", "statement": "Render shell"}],
                }],
                "edges": [],
                "node_hashes": {"file-app": "hash-app"},
                "edge_hashes": {},
                "cursor": "cursor-one",
            }),
            &child_cache_path,
        ...
        cloud_mcp_write_spec_graph_cache(&child_req, &child_snapshot).unwrap();

        let container_req = cloud_mcp_spec_graph_sync_request(
            workspace_path_display(&root),
            Some("workspace-test".to_string()),
            Some("Suite".to_string()),
        );
        let mounts = cloud_mcp_container_project_mounts(&root);
        let snapshot = cloud_mcp_container_spec_graph_snapshot(
            &container_req,
            &mounts,
            "local_mounts",
            "",
        );
        let nodes = snapshot["specNodes"].as_array().cloned().unwrap_or_default();
        let routed_node = nodes
            .iter()
            .find(|node| node["sourceNodeId"].as_str() == Some("file-app"))
            .expect("child node should be included in parent projection");

        assert_eq!(snapshot["containerWorkspace"].as_bool(), Some(true));
        assert_eq!(snapshot["projectMounts"].as_array().map(Vec::len), Some(1));
        assert_ne!(routed_node["id"].as_str(), Some("file-app"));
        assert_eq!(routed_node["sourceRepoId"].as_str(), Some(child_req.repo_id.as_str()));
        assert_eq!(routed_node["sourceGraphCursor"].as_str(), Some("cursor-one"));
        assert_eq!(routed_node["sourceNodeHash"].as_str(), Some("hash-app"));
        assert_eq!(routed_node["mountId"].as_str(), Some("frontend"));
        assert_eq!(routed_node["projectRoot"].as_str(), Some(child_req.root_display.as_str()));
        assert_eq!(routed_node["projectRelativePath"].as_str(), Some("src/App.jsx"));
        assert_eq!(routed_node["visiblePath"].as_str(), Some("frontend/src/App.jsx"));

        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn git_workspace_spec_graph_composes_parent_and_nested_git_child() {
        let root = test_cloud_root("diffforge-cloud-git-nested-spec-graph");
        if !init_cloud_git_repo(&root) {
            let _ = fs::remove_dir_all(root);
            return;
        }
        ...
        let parent_req = cloud_mcp_spec_graph_sync_request(
            workspace_path_display(&root),
            Some("workspace-test".to_string()),
            Some("Suite".to_string()),
        );
        let parent_cache_path =
            cloud_mcp_spec_graph_cache_path(&parent_req.root, &parent_req.repo_id);
        let parent_snapshot = cloud_mcp_spec_graph_snapshot_from_data(
            &parent_req,
            json!({
                "nodes": [
                    {
                    ...
                    },
                    {
                        ...
                        "node_type": "file",
                        "title": "app.js",
                        "path": "packages/mobile/src/app.js"
                    }
                ],
                "edges": [
                    {"id": "edge-parent", "from": "parent-root", "to": "parent-file"},
                    {"id": "edge-stale", "from": "parent-root", "to": "stale-child-file"}
                ],
                "node_hashes": {},
                "edge_hashes": {},
        ...
        cloud_mcp_write_spec_graph_cache(&parent_req, &parent_snapshot).unwrap();

        let child_req = cloud_mcp_spec_graph_sync_request(
            workspace_path_display(&child),
            Some("workspace-test".to_string()),
        ...
        let mount_values = snapshot["projectMounts"].as_array().cloned().unwrap_or_default();
        let nodes = snapshot["specNodes"].as_array().cloned().unwrap_or_default();
        ...
        assert!(mount_values
            .iter()
            .any(|mount| mount["mountId"].as_str() == Some("packages")));
        assert!(mount_values
            .iter()
            .any(|mount| mount["mountId"].as_str() == Some("packages/mobile")));
        assert!(nodes
            .iter()
            .any(|node| node["id"].as_str() == Some("parent-file")));
        assert!(!nodes
            .iter()
            .any(|node| node["id"].as_str() == Some("stale-child-file")));
        assert!(nodes.iter().any(|node| {
            node["sourceNodeId"].as_str() == Some("file-app")
                && node["visiblePath"].as_str() == Some("packages/mobile/src/app.js")
                && node["sourceRepoId"].as_str() == Some(child_req.repo_id.as_str())
        }));
    ...
    #[test]
    fn nested_container_spec_graph_snapshot_groups_projects_under_container_mounts() {
        let root = test_cloud_root("diffforge-cloud-nested-container-spec-graph");
        let frontend = root.join("product-a").join("frontend");
        let backend = root.join("product-a").join("backend");
        create_cloud_package_project(&frontend);
        create_cloud_package_project(&backend);
        ...
        let mount_values = snapshot["projectMounts"].as_array().cloned().unwrap_or_default();
        let nodes = snapshot["specNodes"].as_array().cloned().unwrap_or_default();
        let edges = snapshot["specEdges"].as_array().cloned().unwrap_or_default();
        let product_a_mount = mount_values
            .iter()
            .find(|mount| mount["mountId"].as_str() == Some("product-a"))
            .expect("parent container mount should be present");
        let frontend_mount = mount_values
            .iter()
            .find(|mount| mount["mountId"].as_str() == Some("product-a/frontend"))
            .expect("leaf project mount should be present");
        let product_a_node = nodes
            .iter()
            .find(|node| {
                node["mount_id"].as_str() == Some("product-a")
                    || node["mountId"].as_str() == Some("product-a")
            })
            .expect("container mount node should be present");
        let frontend_node = nodes
            .iter()
            .find(|node| {
                node["mount_id"].as_str() == Some("product-a/frontend")
                    || node["mountId"].as_str() == Some("product-a/frontend")
            })
            .expect("project mount node should be present");

        assert_eq!(product_a_mount["mountKind"].as_str(), Some("container"));
        assert_eq!(frontend_mount["parentMountId"].as_str(), Some("product-a"));
        assert!(edges.iter().any(|edge| {
            edge["from"] == product_a_node["id"] && edge["to"] == frontend_node["id"]
        }));
        assert!(nodes.iter().any(|node| {
            node["sourceNodeId"].as_str() == Some("file-app")
                && node["visiblePath"].as_str() == Some("product-a/frontend/src/App.jsx")
        }));
    ...
    #[test]
    fn container_spec_graph_snapshot_cursor_changes_when_child_cache_changes() {
        let root = test_cloud_root("diffforge-cloud-container-cursor");
        let frontend = root.join("frontend");
        create_cloud_package_project(&frontend);
        ...
        let child_cache_path = cloud_mcp_spec_graph_cache_path(&child_req.root, &child_req.repo_id);
        let first_child_snapshot = cloud_mcp_spec_graph_snapshot_from_data(
            &child_req,
            json!({
                "nodes": [{"id": "file-app", "node_type": "file", "path": "src/App.jsx"}],
                "edges": [],
                "node_hashes": {},
                "edge_hashes": {},
                "cursor": "cursor-one",
        ...
        cloud_mcp_write_spec_graph_cache(&child_req, &first_child_snapshot).unwrap();
        ...
        let mounts = cloud_mcp_container_project_mounts(&root);
        let first_container = cloud_mcp_container_spec_graph_snapshot(
            &container_req,
            &mounts,
            "local_mounts",
            "",
        ...
        let second_child_snapshot = cloud_mcp_spec_graph_snapshot_from_data(
            &child_req,
            json!({
                "nodes": [{"id": "file-app", "node_type": "file", "path": "src/App.jsx"}],
                "edges": [],
        ...
        cloud_mcp_write_spec_graph_cache(&child_req, &second_child_snapshot).unwrap();
        let second_container = cloud_mcp_container_spec_graph_snapshot(
            &container_req,
            &mounts,
            "local_mounts",
            "",
        ...
        assert_ne!(
            first_container["cursor"].as_str(),
            second_container["cursor"].as_str()
        );
    ...
    #[tokio::test]
    async fn child_spec_graph_sync_recomposes_active_parent_container_snapshot() {
        let root = test_cloud_root("diffforge-cloud-container-active-recompose");
        let frontend = root.join("frontend");
        create_cloud_package_project(&frontend);
        ...
        let generation = 42;
        {
            let mut container_syncs = state.spec_graph_container_syncs.lock().await;
            container_syncs.insert(
                container_req.repo_id.clone(),
                CloudMcpSpecGraphContainerRuntime {
                    generation,
                    req: container_req.clone(),
                    mounts: mounts.clone(),
                },
            );
        }

        let snapshots = cloud_mcp_container_snapshots_for_child_sync(
            &state,
            &child_req.root,
            "ready",
            "",
        ...
        assert_eq!(snapshots[0]["syncGeneration"].as_u64(), Some(generation));
        assert_eq!(
            snapshots[0]["lastChildRepoPath"].as_str(),
            Some(child_req.root_display.as_str())
        );
        assert!(snapshots[0]["specNodes"]
            .as_array()
            .map(|nodes| {
                nodes
                    .iter()
                    .any(|node| node["sourceNodeId"].as_str() == Some("file-app"))
            })
            .unwrap_or(false));
    ...
    fn filetree_progressive_payloads_start_with_root_then_depth_layers() {
        ...
        assert_eq!(final_payload["filetree_authoritative"].as_bool(), Some(true));
    ...
    fn container_filetree_progressive_payloads_stay_mount_only() {
        ...
        assert_eq!(payloads[0]["filetreeRoot"]["kind"].as_str(), Some("container"));
        ...
        assert_eq!(final_payload["filetree_complete"].as_bool(), Some(true));
        assert_eq!(final_payload["filetree_authoritative"].as_bool(), Some(false));
    ...
    fn broad_area_filetree_payloads_are_root_only_and_non_authoritative() {
        ...
        assert!(!cloud_mcp_workspace_kind_filetree_authoritative("broad_area"));
    ...
    fn plain_unknown_workspace_bundle_is_discovery_only_not_authoritative_filetree() {
        ...
        fs::write(root.join("private-notes.txt"), "do not sync this filename as a project\n")
            .unwrap();
    ...
    fn filetree_sync_epochs_are_distinct_and_batches_are_ordered() {
        ...
        assert!(payloads
            .iter()
            .all(|payload| payload["filetree_sync_epoch_id"].as_str() == Some(first.epoch_id.as_str())));
    }
