...
- Synced architecture files live under the single account-global `globalGraphsRoot`. Call `coordination-kernel.architecture_context` (or read `DIFFFORGE_ARCHITECTURE_GRAPHS_ROOT`) for the exact absolute path; do not assume the graphs live inside the repo working tree.
- Account-global architecture files are stored under `<globalArchitecturesRoot>/.agents/architectures/graphs/*.arch`. Graphs written there are not tied to any repo: they sync at account level and appear on every signed-in device's Architectures tab.
- Use the DSL `folder "Area / Subarea"` line to place a graph under named folders in the Architectures sidebar; do not create repo-scoped or pseudo-repo architecture roots.
- Local-only revision snapshots live under the account-global `.agents/architectures/revisions/<graph-id>/`. They are never included in normal latest graph context.
- `.agents/architectures/index.json` is generated by Diff Forge inside the account-global architecture root.
- `.agents/architectures/AGENTS.md` and `.agents/architectures/icon-aliases.json` are generated reference files for agents inside the account-global architecture root.
- `globalGraphsRoot/*.arch` graph source files are live-preview artifacts and may be edited directly. For code and docs, follow the active Diff Forge session file authority: leased direct edits use the visible repo root, while isolated worktree sessions use the assigned branch root.

When a user asks for an architecture, diagram, system map, deployment view, data flow, auth flow, billing/subscription flow, control graph, state machine, dependency graph, API pathway, API corridor, or subsystem visualization, create or update a `.arch` graph in `globalGraphsRoot`.
...
Call `coordination-kernel.architecture_context` before architecture, diagram, deployment, flow, or subsystem visualization work. Use `coordination-kernel.architecture_list` to discover existing graphs and `coordination-kernel.architecture_icon_reference` when choosing icons. Create or modify `globalGraphsRoot/*.arch` with normal file edits so the Architecture tab reloads file changes live.

Architecture graph-only work is live artifact work, not a managed patch task. If the only edits are `globalGraphsRoot/*.arch`, do not call `coordination-kernel.start_task`, do not acquire a normal file lease, and do not call `coordination-kernel.submit_patch`.
...
pub(crate) fn architecture_icon_reference_value(repo_path: String) -> Result<Value, String> {
    ...
    if let Some(object) = reference.as_object_mut() {
        ...
        object.insert(
            ...
            Value::String(workspace_path_display(&icon_reference_path)),
        );
    }
    Ok(reference)
}
...
pub(crate) fn architecture_context_value(repo_path: String) -> Result<Value, String> {
    ...
    Ok(json!({
        ...
        "storageScope": "account-global",
        "storageNote": "Synced architecture graphs live in the account-global store; use folder \"Area / Subarea\" inside the DSL for named folders.",
        "architectureRoot": workspace_path_display(&architecture_root),
        "graphsRoot": workspace_path_display(&graphs_root),
        ...
        "agentGuidePath": workspace_path_display(&guide_path),
        "iconReferencePath": workspace_path_display(&icon_reference_path),
        "globalArchitecturesRoot": Some(workspace_path_display(&storage_root)),
        "globalGraphsRoot": Some(workspace_path_display(&graphs_root)),
        "globalScope": {
            "purpose": "Account-global architecture graphs. They sync at account level and appear on every signed-in device's Architectures tab.",
            "createWhen": "Use this for all synced architecture graphs.",
            "howTo": "Write .arch files into globalGraphsRoot. Put folder \"Area / Subarea\" near the top of the DSL to place the graph under named nested folders.",
        },
        "sourceFormat": "eraserDsl",
        "graphExtension": ".arch",
        "graphs": graphs,
        "contract": {
            "purpose": "Architecture graphs are account-global system graph artifacts for humans and agents.",
            "preferredMcpTools": [
            ...
            "workflow": [
                ...
                "Write synced architecture .arch files into globalGraphsRoot; all architecture sync is account-global.",
                "Use the DSL folder line, for example folder \"Backend / API\", for named and nested architecture folders.",
                "For architecture graph-only work, do not call start_task, acquire_lease, checkpoint, or submit_patch; globalGraphsRoot/*.arch files are direct live artifacts.",
                "Read the closest existing globalGraphsRoot/*.arch file directly before modifying a known subsystem.",
                "Create or update globalGraphsRoot/*.arch with normal file edits using the eraser-like DSL.",
...
-> Result<(PathBuf, PathBuf, PathBuf, PathBuf, PathBuf), String> {
    ...
    fs::create_dir_all(&graphs_root)
        .map_err(|error| format!("Unable to create global architecture graph directory: {error}"))?;
