...
- Account-global architecture files live in the device's Diff Forge global architectures root, under `<globalArchitecturesRoot>/.agents/architectures/graphs/*.arch`. Call `coordination-kernel.architecture_context` and read `globalArchitecturesRoot` or `globalGraphsRoot` for the exact path. Graphs written there are not tied to any repo: they sync at account level and appear on every signed-in device's Architectures tab.
- All synced architecture graphs live under the single account-global `globalGraphsRoot`. Use the DSL `folder "Area / Subarea"` line to place a graph under named folders in the Architectures sidebar; do not create repo-scoped or pseudo-repo architecture roots.
