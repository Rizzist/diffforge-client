...
pub(crate) fn architecture_orphan_root_dir(identity_key: &str) -> Result<PathBuf, String> {
    ...
    let root = cloud_mcp_local_data_file_path("architectures")
        .ok_or_else(|| "Orphan architectures root is unavailable.".to_string())?
        .join("orphans")
        .join(slug);
    fs::create_dir_all(&root)
        .map_err(|error| format!("Unable to create orphan architectures root: {error}"))?;
    Ok(root)
}
...
async fn architecture_graph_copy(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let source = resolve_workspace_root_directory(Some(source_repo_path.as_str()))?;
        let target = resolve_workspace_root_directory(Some(target_repo_path.as_str()))?;
