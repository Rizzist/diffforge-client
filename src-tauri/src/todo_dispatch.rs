...
pub(crate) fn todo_dispatch_record_receipt_internal(
    ...
    let workspace_name = merged
        ...
        .unwrap_or_default()
        .to_string();
    let notify_completion_is_fresh =
        status != "completed" || todo_dispatch_completed_receipt_fresh_for_notification(&Value::Object(merged.clone()));
    ...
    if let Some(app) = app {
        ...
        if before_active > 0 && after_active == 0 && status == "completed" && notify_completion_is_fresh {
...
pub(crate) fn todo_dispatch_record_remote_intake(app: &AppHandle, event: &Value) -> Option<Value> {
    ...
    let origin_workspace_id = todo_dispatch_text(event, &["origin_workspace_id", "originWorkspaceId"]);
                    ...
                    if matches!(
                        ...
                        if todo_dispatch_remote_intake_already_current(
                            ...
                            if let Some(object) = existing.as_object_mut() {
                                ...
                                object
                                    .insert("updatedAtMs".to_string(), json!(todo_dispatch_now_ms()));
                            ...
                            todo_store_set_item_status(existing, intake_status, "remote_todo_intake");
...
fn todo_dispatch_todo_sync_commit_payload(
    ...
    if let Some(meta_object) = meta.as_object_mut() {
        if !origin_client_id.is_empty() {
            meta_object.insert("origin_client_id".to_string(), json!(origin_client_id.clone()));
            meta_object.insert("originClientId".to_string(), json!(origin_client_id.clone()));
        }
        if !origin_device_id.is_empty() {
            meta_object.insert("origin_device_id".to_string(), json!(origin_device_id.clone()));
            meta_object.insert("originDeviceId".to_string(), json!(origin_device_id.clone()));
            meta_object.insert("request_device_id".to_string(), json!(origin_device_id.clone()));
...
fn todo_dispatch_core_terminal_ready_for_submit(
    ...
    let readiness = terminal_projection_text(&projected.readiness, "");
    let lifecycle = terminal_projection_text(&projected.terminal_lifecycle, "");
    let status = terminal_projection_text(&projected.terminal_status, "");
    let work_state = terminal_projection_text(&projected.terminal_work_state, "");
    ...
    if parked
        || terminal_projection_state_is_closed(&lifecycle)
        ...
        || matches!(
            readiness.as_str(),
            "busy" | "closing" | "closed" | "error" | "needs_input"
        )
        || matches!(
            work_state.as_str(),
            "closed" | "error" | "prompting_user" | "running"
        )
    {
        return Some(false);
    }
    ...
    if runtime.input_ready
        || (readiness == "ready"
            && work_state == "complete"
            && (terminal_projection_state_is_idle(&status)
...
mod todo_dispatch_backend_tests {
    ...
    fn backend_submit_readiness_accepts_core_idle_projection_without_input_ready_bit() {
        let runtime = backend_submit_runtime("active", "idle", "ready", false);
...
fn todo_dispatch_backend_ready_entry_allows_submit(entry: &Value) -> bool {
    ...
    if terminal_projection_state_is_busy(&status)
        ...
        || matches!(
            work_state.as_str(),
            "running" | "prompting_user" | "closed" | "error"
        )
    {
        return false;
    }
...
fn todo_dispatch_normalized_terminal_name(value: &str) -> String {
    value
        .split_whitespace()
        .collect::<Vec<_>>()
        .join(" ")
...
fn todo_dispatch_backend_entry_matches_name(entry: &Value, target_name: &str) -> bool {
    let target_name = todo_dispatch_normalized_terminal_name(target_name);
    ...
    .iter()
    .any(|key| todo_dispatch_normalized_terminal_name(&todo_dispatch_text(entry, &[*key])) == target_name)
}

fn todo_dispatch_backend_pick_target_from_entries(entries: &[Value], item: &Value) -> Option<Value> {
    if entries.is_empty() {
        return None;
    }
    ...
    let has_explicit_target = !target_pane.is_empty()
    ...
    let matched = entries
        ...
        .or_else(|| {
            target_index.and_then(|target_index| {
                entries.iter().find(|entry| {
                    todo_dispatch_i64(
                        entry,
                        &[
        ...
        .or_else(|| {
            if target_name.is_empty() {
                ...
                entries
                    .iter()
                    .find(|entry| todo_dispatch_backend_entry_matches_name(entry, &target_name))
            }
        })
        .or_else(|| {
            if has_explicit_target {
                // Explicit terminal selectors must resolve exactly; otherwise
                // do not fall back to an arbitrary ready pane.
                None
